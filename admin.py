...
def get_tool():
    button = [
        [InlineKeyboardButton(text="Добавить новый продукт", callback_data="add_product")]
    ]
...
def get_catalog():
    buttons = [
        [
            InlineKeyboardButton(text='📱 Телефоны и ⌚ гаджеты', callback_data='edit_phone_gadget'),
            InlineKeyboardButton(text='🏠 Бытовая техника', callback_data='edit_appliances')
        ],
        [
            InlineKeyboardButton(text='🔊 Аудио, 📹 Видео', callback_data='edit_periphery'),
            InlineKeyboardButton(text='💻 Компьютеры', callback_data='edit_comp')
        ],
        [
            InlineKeyboardButton(text='📺 Телевизоры', callback_data='edit_tv'),
            InlineKeyboardButton(text='🧰 Строительсто и ремонт', callback_data="edit_sport")
        ],
        [
...
def get_catalog_phone():
    buttons = [
        [
            InlineKeyboardButton(text='📱 Смартфоны', callback_data='uedit_phone'),
            InlineKeyboardButton(text='⌚ Смарт часы', callback_data='uedit_watch')
        ],
        [
            InlineKeyboardButton(text='🔌 Адаптеры', callback_data='uedit_charger'),
            InlineKeyboardButton(text='📲 Чехлы', callback_data="uedit_case")
        ]
    ]
...
def isAdmin(user_id):
    return user_id == config.admin_id.get_secret_value()
...
async def choose_catalog(callback: CallbackQuery):
    if isAdmin(callback.message.from_user.id):
...
async def get_photo(message: Message, state: FSMContext):
    ...
    await insert_product(product["choose_name"], product["choose_price"], product["choose_characteristics"], product["choose_photo"])
    await state.clear()
